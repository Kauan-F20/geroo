...
export default function criarMetrica() {
  ...
  const getIdoso = async () => {
    ...
    try {
      ...
      console.log("Dados do idoso recebidos:", data);
  
      // Supondo que a API retorne um único objeto para o idoso
      const idosoPayload = {
        ...data,
        foto: getImageUri(data.foto),
      };
  
      // Salva no estado e também no armazenamento local
      setIdoso(idosoPayload);
      await AsyncStorage.setItem("idoso", JSON.stringify(idosoPayload));
  
      console.log("Idoso carregado e salvo localmente:", idosoPayload);
    } catch (err) {
      console.error("Erro ao buscar dados remotos:", err);
  
      // Fallback: tenta carregar do armazenamento local
      AsyncStorage.getItem("idoso").then((idosoString) => {
        if (idosoString) {
          const idosoPayload = JSON.parse(idosoString) as IIdoso;
          setIdoso(idosoPayload);
