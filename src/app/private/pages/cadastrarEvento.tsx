...
export default function CadastrarEvento() {
  ...
  const getIdoso = async () => {
    ...
    try {
      // Tenta buscar os dados do banco remoto
      const response = await fetch(`${BASE_URL}/idoso`, {
        method: "GET",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Erro ao carregar os dados do idoso.");
      }
  
      const data = await response.json();
  
      console.log("Dados do idoso recebidos do servidor:", data);
  
      // Processa os dados recebidos
      const idosoPayload = {
        ...data,
        foto: getImageUri(data.foto), // Transformação na foto, se necessário
      };
  
      // Salva os dados no estado e no armazenamento local
      setIdoso(idosoPayload);
      await AsyncStorage.setItem("idoso", JSON.stringify(idosoPayload));
  
      console.log("Idoso carregado e salvo no armazenamento local:", idosoPayload);
    } catch (err) {
      console.error("Erro ao buscar dados remotos:", err);
  
      // Fallback: busca no armazenamento local
      AsyncStorage.getItem("idoso").then((idosoString) => {
        if (idosoString) {
          const idosoPayload = JSON.parse(idosoString) as IIdoso;
          setIdoso(idosoPayload);
          console.log("Dados do idoso carregados do armazenamento local:", idosoPayload);
        } else {
          Toast.show({
            type: "error",
            text1: "Erro!",
            text2: "Não foi possível carregar os dados do idoso.",
          });
        }
      });
    } finally {
